...
class Chatprocess extends Component {
  ...
  getAllMessage = (item) => {
    var dataUser = JSON.parse(localStorage.getItem('userInfo'))
    axios
      .get(`/api/chat/views/${item}/${dataUser[0].id}`)
      .then((res) => {
        if (res.status === 200) {
  ...
  handleSendMessage = (event) => {
    ...
    axios
      .post(
        `/api/chat/insert/${dataUser[0].id}/${this.state.id_account}`,
        newMessage
      )
      .then((res) => {
                ...
                <form onSubmit={this.handleSendMessage}>
                  <div>
                    <header>
                      ...
                      />
                      <div>
                        <h2 name="chat_from" onChange={this.handleInputChange}>
                          Trò chuyện với {this.state.name}
                        </h2>
                  ...
                  {this.state.news.map((item, key) => (
                    <button
                      onClick={() =>
                        ...
                        this.getAllMessage(item.id_account) ||
                        this.componentWillMount(item.id_account)
                      }
                      data-target="#chat"
                      key={key}
                    >
